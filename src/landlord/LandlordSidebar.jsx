...
export function LandlordSidebar({ user, verified = false, activeSection, unreadNotifications = 0, onSectionChange, onClose, onLogout }) {
    ...
    return (<div className="app-sidebar">
      ...
      <nav className="landlord-sidebar-nav"><p className="landlord-sidebar-main">Main<span className="landlord-sidebar-main-2"/></p><div className="landlord-sidebar-panel-2">{mainItems.map(({ label, section, icon: Icon }) => <button key={section} aria-current={activeSection === section ? "page" : undefined} onClick={() => selectSection(section)} className={navClass(activeSection === section)}><Icon className="landlord-sidebar-icon-icon"/>{label}{section === "notifications" && unreadNotifications > 0 && <span className="app-sidebar-badge">{unreadNotifications}</span>}</button>)}</div></nav>
      <nav className="landlord-sidebar-nav-2"><Link to="/browse" onClick={onClose} aria-current={activeSection === "market" ? "page" : undefined} className={navClass(activeSection === "market")}><TrendingUp className="landlord-sidebar-trending-up-icon"/>Market Trends</Link></nav>
