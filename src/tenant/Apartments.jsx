...
import { hasMeaningfulPreferences } from "@/tenant/rankingEngine";

import { getRecommendedApartments } from "@/tenant/recommended";
...
import {
    ...
    formatDistance,
    parseNearbySearchIntent,
} from "@/tenant/geospatialSearch";
...
const getAvailableRoomPrices = (apartment) =>
    (apartment.rooms ?? [])
        .filter((room) => {
            const status =
...
const getApartmentPriceLabel = (apartment) => {
    ...
    return `${formatPrice(lowestPrice)} - ${formatPrice(highestPrice)}/month`;
};
...
function TenantBrowse() {
    ...
    const [roomCapacity, setRoomCapacity] = useState(
        defaultTenantPreferences.roomCapacity || "any"
    );

    const [sortBy, setSortBy] = useState(
        defaultTenantPreferences.sortBy || "recommended"
    );
    ...
    const [preferencesOpen, setPreferencesOpen] = useState(
        searchParams.get("preferences") === "open"
    );
    ...
    const hasPersonalizationPreferences =
        isTenantRole(user?.role) && hasMeaningfulPreferences(savedPreferences);
    ...
    // ==========================================================
    // LOAD SAVED TENANT PREFERENCES
    //
    // Gin-islan:
    // Kung may saved preferences na ang tenant,
    // automatic nga Recommended ang makita pagkatapos refresh.
    // ==========================================================
    ...
    useEffect(() => {
        ...
        void fetchTenantPreferences(tenantId)
            .then((preferences) => {
                 ...
                 *
                 * Previously:
                 * browseMode remained "all".
                 *
                 * All Apartments uses randomized
                 * apartment order.
                 *
                 * Now:
                 * If the tenant has saved
                 * personalization preferences,
                 * automatically use Recommended.
                 */
    ...
    useEffect(() => {
        ...
        const unsubscribe = subscribeToApartmentRatings(() => {
            void loadRankingData();
        });
    ...
    const getViewCount = (apartmentId) =>
        viewRows
            .filter(
                (view) =>
                    (view.apartment_id ?? view.apartmentId) === apartmentId
            )
            .reduce(
                (total, view) =>
                    total + Math.max(0, Number(view.view_count) || 0),
                0
            );


    const getFavoriteCount = (apartmentId) =>
        favoriteRows.filter(
            (favorite) =>
                (favorite.apartment_id ?? favorite.apartmentId) === apartmentId
        ).length;


    const viewLabel = (count) =>
    ...
    const landlordById = useMemo(
        () =>
            new globalThis.Map(
                users.filter((row) => row.id).map((row) => [row.id, row])
            ),
        [users]
    );
    ...
    const getLandlord = (apartment) =>
        apartment.landlordId
            ? landlordById.get(apartment.landlordId)
            : undefined;
    ...
    const nearbyIntent = useMemo(
        () => parseNearbySearchIntent(searchQuery),
        [searchQuery]
    );
    ...
    const submitSearch = async (event) => {
        ...
        try {
            const location = await geocodeLocationWithinLaPaz(
                intent.target,
                controller.signal
            );

            if (request === geocodeRequest.current) {
                setActiveNearbySearch({
            ...
            if (request === geocodeRequest.current) {
                setNearbySearchError(
                    error instanceof GeocodingError
                        ? error.message
                        : "Unable to search that location."
                );
            }
        } finally {
            if (request === geocodeRequest.current) {
     ...
     * Ginabilin naton ini.
     *
     * All Apartments = randomized.
     *
     * Recommended will still pass through
     * rankApartments(), so the ranking engine
     * determines its final order.
     */
    const randomizedApartments = useMemo(
        () => shuffleApartments(allApartments),
        [allApartments, shuffleVersion]
    );


    const filteredApartments = useMemo(() => {
        const sourceApartments =
            browseMode === "all" ? randomizedApartments : allApartments;

        const filtered = sourceApartments.filter((apt) => {
            ...
            if (!matchesRoomCapacity(apt, roomCapacity)) {
                return false;
            }
        ...
        if (browseMode === "popular") {
            return [...filtered]
                .filter(
                    (apartment) =>
                        getViewCount(apartment.id) > 0 ||
                        getFavoriteCount(apartment.id) > 0
                )
                .sort((a, b) => {
                    const aEngagement =
                        getViewCount(a.id) + getFavoriteCount(a.id) * 2;

                    const bEngagement =
                        getViewCount(b.id) + getFavoriteCount(b.id) * 2;

                    return (
                        bEngagement - aEngagement ||
        ...
        // ==========================================
        // RECOMMENDED
        // Sorted only by ranking score
        // ==========================================

        if (browseMode === "suggested") {
            return getRecommendedApartments({
                apartments: filtered,
                savedPreferences,
                userRole: user?.role,
            });
    ...
    const activeFilterCount = [
        savedPreferences.preferredArea?.trim(),

        savedPreferences.minBedrooms !== "any",
        ...
        ...[
            ...
            "ac",
            "laundryArea",
        ].map((key) => savedPreferences[key]),
    ].filter(Boolean).length;


    const mappedApartmentCount = filteredApartments.filter((apartment) =>
        hasValidApartmentCoordinates(apartment.lat, apartment.lng)
    ).length;
    ...
    const hasActiveApartmentFilters = Boolean(
        searchQuery.trim() || activeNearbySearch || activeFilterCount > 0
    );
    ...
    //
    // Gin-islan:
    // After mag-save ang tenant, automatic nga Recommended.
    // ==========================================================
    ...
    const saveBrowsePreferences = async (draft) => {
         ...
         * IMPORTANT CHANGE:
         *
         * After saving Preferred Location,
         * Budget, Amenities, etc.,
         * automatically switch to Recommended.
         */
        setBrowseMode("suggested");
    ...
    const renderFilterTrigger = (floating = false) => (
        <DialogTrigger asChild>
            <Button
                ...
                className={
                    floating
                        ? "apartment-browse-button"
                        : "apartment-browse-button-2"
                }
            ...
            >
                <SlidersHorizontal
                    className={
                        floating
                            ? "apartment-browse-sliders-horizontal-icon"
                            : "apartment-browse-sliders-horizontal-icon-2"
                    }
                />
    ...
    const ApartmentsCard = ({ apartment }) => {
        ...
        return (
            <article className="apartment-browse-article">
                <div className="apartment-browse-panel-5">
                    {imageUrl ? (
                        <ImageWithFallback
                            src={imageUrl}
                            alt={apartment.title}
                            className="apartment-browse-image-with-fallback"
                        />
                    ) : (
                        <div className="apartment-browse-row-6">
                    ...
                    <div className="apartment-browse-content-2">
                        {activeNearbySearch &&
                            apartment.distanceMeters !== undefined && (
                                <Badge className="apartment-browse-badge">
                                    {formatDistance(apartment.distanceMeters)}
                                </Badge>
                            )}

                        <Badge
                            className={`apartment-browse-badge-2 ${
                        ...
                        {isVerifiedListing(apartment) && (
                            <VerifiedBadge
                                label="Verified Listing"
                                className="apartment-browse-verified-badge"
                            />
                        )}

                        {apartment.petFriendly && (
                            <Badge className="apartment-browse-pet-friendly">
                                Pet Friendly
                            </Badge>
                        )}
                    </div>
                    ...
                    <button
                        ...
                        className={`apartment-browse-button-6 ${
                            favorite
                                ? "apartment-browse-button-7"
                                : "apartment-browse-button-8"
                        }`}
                    >
                        {favoriteUpdating ? (
                            <RefreshCw className="apartment-browse-refresh-cw-icon" />
                        ) : (
                            <Heart
                                className="apartment-browse-heart-icon"
                ...
                <div className="apartment-browse-panel-6">
                    <div className="apartment-browse-row-7">
                        <div className="apartment-browse-panel-7">
                            <h2 className="apartment-browse-heading">
                                {apartment.title}
                            </h2>
                            ...
                            <p className="apartment-browse-text-3">
                                <MapPin className="apartment-browse-map-pin-icon" />
                        ...
                        <div className="apartment-browse-panel-8">
                            <p className="apartment-browse-view-room-prices">
                                {getApartmentPriceLabel(apartment)}
                            </p>

                            <p className="apartment-browse-text-4">
                                <Eye className="apartment-browse-eye-icon" />
                    ...
                    <div className="apartment-browse-grid-5">
                        ...
                        <Metric
                            icon={Bed}
                            label="bed"
                            value={apartment.bedrooms.toLocaleString()}
                        />

                        <Metric
                            icon={Bath}
                            label="bath"
                            value={apartment.bathrooms.toLocaleString()}
                        />

                        <Metric
                            icon={Square}
                            label="sqft"
                            value={Number(apartment.sqft || 0).toLocaleString()}
                        />
                    </div>
                    ...
                    <Button
                        asChild
                        variant="outline"
                        className="apartment-browse-button-9"
                    >
        ...
        <Dialog open={preferencesOpen} onOpenChange={setPreferencesOpen}>
            <div className="tenant-browse">
                ...
                <div className="apartment-browse-row-8">
                    <div className="apartment-browse-panel-9">
                        <Sidebar
                            active="apartments"
                            unreadCount={unreadCount}
                        />
                    ...
                    <main className="app-shell-main">
                        <div className="app-shell-content app-shell-content-mobile-nav">
                            ...
                            {/* ====================================================
                                1. HEADER CARD (greeting + title + count only)
                            ==================================================== */}
                            <section className="apartment-browse-section-3">
                                <div className="apartment-browse-panel-11">
                                    ...
                                    <p className="apartment-browse-text-7">
                                        {activeNearbySearch
                                            ? `${filteredApartments.length} apartments within 500 m of ${activeNearbySearch.target} · nearest first`
                            ...
                            {/* ====================================================
                                2. SEARCH BAR + PREFERENCES (one row, below header)
                            ==================================================== */}
                            <form
                                onSubmit={submitSearch}
                                className="apartment-browse-form-hero"
                            >
                                <div className="apartment-browse-grid-6">
                                    <div className="apartment-browse-panel-10">
                                        ...
                                        <input
                                            ...
                                            placeholder="Search apartments or try near ISAT U"
                                            className="apartment-browse-input-2"
                                        />
                                    </div>
                                ...
                                {nearbySearchLoading && (
                                    <p className="apartment-browse-finding-nearby-apartments">
                                        Finding nearby apartments…
                                    </p>
                                )}

                                {nearbySearchError && (
                                    <p className="apartment-browse-text-5">
                                        {nearbySearchError}
                                    </p>
                                )}
                            </form>
                            ...
                            {/* ====================================================
                                3. TABS (left) + BROWSE | MAP TOGGLE (right)
                            ==================================================== */}
                            <div className="apartment-browse-section-4">
                                <div className="apartment-browse-row-9">
                                    <BrowseButton
                                        label="All Apartments"
                                        active={browseMode === "all"}
                                        onClick={() => {
                                            if (browseMode !== "all") {
                                                setShuffleVersion(
                                                    (current) => current + 1
                                    ...
                                    <BrowseButton
                                        label="Popular"
                                        active={browseMode === "popular"}
                                        onClick={() => setBrowseMode("popular")}
                                    />

                                    <BrowseButton
                                        label="Recommended"
                                        active={browseMode === "suggested"}
                                        onClick={() =>
                                            setBrowseMode("suggested")
                                        }
                                    />
                                ...
                                <div className="apartment-browse-grid-7">
                                    <button
                                        ...
                                        className={`apartment-browse-grid-8 ${
                                            viewMode === "grid"
                                                ? "apartment-browse-grid-9"
                                                : "apartment-browse-grid-10"
                                        }`}
                                    >
                                        <Grid2X2 className="apartment-browse-grid2-x2-icon" />
                                        Browse
                                    </button>
                                    ...
                                    <button
                                        ...
                                        className={`apartment-browse-map ${
                                            viewMode === "map"
                                                ? "apartment-browse-map-2"
                                                : "apartment-browse-map-3"
                                        }`}
                                    >
                                        <Map className="apartment-browse-map-icon" />
                                        Map
                                    </button>
                                </div>
                            </div>
                            ...
                            <section className="apartment-browse-section-5">
                                {apartmentsRefreshing &&
                                    allApartments.length > 0 && (
                                        <div className="apartment-browse-refreshing-latest-apartment-data">
                                            <RefreshCw
                                                className="apartment-browse-refresh-cw-icon-2"
                                                aria-hidden="true"
                                            />
                                ...
                                allApartments.length === 0 ? (
                                    <div
                                        className="apartment-browse-loading-apartments"
                                    ...
                                    >
                                        {Array.from({ length: 6 }).map(
                                            (_, index) => (
                                                <div
                                                    key={index}
                                                    className="apartment-browse-card-3"
                                                >
                                  ...
                                  allApartments.length === 0 ? (
                                    <div className="apartment-browse-card-4">
                                        ...
                                        <Button
                                            ...
                                            className="apartment-browse-try-again"
                                        >
                                            Try Again
                                        </Button>
                                    </div>
                                ) : filteredApartments.length === 0 ? (
                                    <div className="apartment-browse-card-5">
                                        <LocateFixed className="apartment-browse-locate-fixed-icon" />

                                        <h2 className="apartment-browse-no-apartments-found">
                                        ...
                                        {hasActiveApartmentFilters && (
                                            <Button
                                                ...
                                                className="apartment-browse-reset-search-amp-filters"
                                            >
                                    ...
                                    <div className="apartment-browse-card-6">
                                        <div className="apartment-browse-shown-on-map">
                                            {mappedApartmentCount.toLocaleString()}{" "}
                                            {mappedApartmentCount === 1
                                                ? "apartment"
                                                : "apartments"}{" "}
                                            shown on map

                                            {mappedApartmentCount <
                                                filteredApartments.length && (
                                                <span className="apartment-browse-span-2">
                                                    Some listings could not be
                                                    placed on the map.
                                                </span>
                                            )}
                                        </div>
                                        ...
                                        <MapView
                                            ...
                                            lng={mapCenter.lng}
                                            zoom={12}
                                            apartments={filteredApartments.map(
                                                (apt) => ({
                                                    id: apt.id,
...
function Metric({ icon: Icon, value, label }) {
    return (
        <div className="apartment-browse-row-10">
...
function BrowseButton({ label, active, onClick }) {
    return (
        <button
            ...
            onClick={onClick}
            aria-pressed={active}
            className={`apartment-browse-button-10 ${
                active
                    ? "apartment-browse-button-11"
                    : "apartment-browse-button-12"
            }`}
        >
            {label}
        </button>
...
function Pagination({
    ...
    return (
        <div className="apartment-browse-content-3">
            <p className="apartment-browse-showing">
                Showing {totalItems === 0 ? 0 : pageStart + 1} to{" "}
                {pageStart + pageCount} of {totalItems} apartments
            </p>
            ...
            <div className="apartment-browse-row-14">
                <button
                    ...
                    className="apartment-browse-button-13"
                >
                ...
                {visiblePages.map((page) => (
                    <button
                        ...
                        className={`apartment-browse-button-14 ${
                            currentPage === page
                                ? "apartment-browse-button-15"
                                : "apartment-browse-button-16"
                        }`}
                    >
                        {page}
                    </button>
                ...
                <select
                    ...
                    className="apartment-browse-select"
                >
